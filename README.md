<h1 align="center">Ahmed Jaber</h1>
<h3 align="center">Mobile App Developer — Flutter & Android Specialist</h3>

<p align="center">
  I build production-grade mobile apps — from offline-first architectures to AI-powered features — and coach the next generation of Flutter developers.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/ahmed-jaber-ahmed-779360258" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="https://g.dev/Ahmed_Jaber" target="_blank">
    <img src="https://img.shields.io/badge/Google%20Dev-4285F4?style=for-the-badge&logo=google&logoColor=white" alt="Google Dev"/>
  </a>
  <a href="https://drive.google.com/file/d/1yDrYyUJySgyCBu9SOv4HjUHu8iQSKUbz/view?usp=sharing" target="_blank">
    <img src="https://img.shields.io/badge/Resume-FF6F61?style=for-the-badge&logo=readdotcv&logoColor=white" alt="Resume"/>
  </a>
</p>

---

### 🎯 What I Do

I'm a Computer Science & AI Graduated from MTI University, currently working as a **Flutter Coaching Instructor** for the Ministry of Communications & IT's Digital Egypt Pioneers Initiative (DEPI) — mentoring multiple trainee cohorts through Flutter, Bloc/Cubit, clean architecture, and Firebase.

My focus is shipping mobile apps that don't just work in a demo — they hold up offline, sync reliably, and integrate real ML models where it matters.

---

### 🏆 Flagship Project — ClassTrack

**AI-powered university attendance & performance system**, built as my graduation project with an 8-person team — submitted to **AYAIR 2026** (Presidential African Youth in AI and Robotics).

- 🔐 10-second HMAC-signed QR tokens — kills proxy attendance
- 🧠 On-device face verification: YOLOv8 + FaceNet (512-D embeddings) via a Flask API — **94.3% TAR, <0.1% FAR**
- 📡 Offline-first with SQLite → Supabase (PostgreSQL + RLS) sync
- 📈 Linear regression grade predictor (R² 0.847) flags at-risk students **5 weeks earlier** than midterm-based methods
- ✅ 47/47 test cases passed, 83.2% coverage, validated in a live pilot at MTI University

*Four role-based apps: Student, Faculty Dashboard, T.A., Admin — full CRUD, analytics, PDF/Excel export.*

---

### 💼 Selected Work

| Project | What it does |
|---|---|
| **Hirely** | Arabic-first freelance marketplace — full RTL, real-time chat, PayPal escrow flow |
| **Hotel Booking (Dual App)** | User + Admin apps, Clean Architecture/MVVM, PayPal checkout |
| **Clinic Appointment System** | Doctor scheduling + booking, same architecture family as Hotel Booking |
| **AI Vision App** | Auto-generates product descriptions from photos for small business owners |
| **E-Commerce + Admin Dashboard** | Stripe/PayPal checkout, cut checkout time by 35% |

*More apps — social platform, real-time chat, food delivery, and utility apps — in the repos below.*

---

### 🧰 Tech I Work With

**Languages** — Dart · Java · Kotlin · Python · C++
**Mobile** — Flutter · Native Android
**Architecture** — Clean Architecture · MVVM · Repository Pattern
**State Management** — Bloc · Cubit · Provider
**Backend** — Supabase (Postgres/Realtime/RLS) · Firebase · Flask REST APIs
**AI/ML** — YOLOv8 · FaceNet · TFLite · Linear Regression
**Data** — SQLite · Isar · PostgreSQL

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=AhmedJaberAhmed&show_icons=true&theme=radical&hide=issues" alt="Ahmed Jaber's GitHub stats" width="48%"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=AhmedJaberAhmed&layout=compact&theme=radical" alt="Top Languages" width="42%"/>
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=AhmedJaberAhmed&theme=gruvbox" alt="GitHub Contribution Graph" width="90%"/>
</p>

---

<p align="center">📫 Open to Flutter/mobile roles and collaborations — reach out on LinkedIn.</p>
